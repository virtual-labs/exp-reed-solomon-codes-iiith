...
- Perform row operations on matrix A to convert it into its Reduced Row Echelon Form (RREF). Remember that arithmetic operations should be performed in the finite field $\mathbb{F}{_q}$.
...
Now with above-mentioned procedure we will check $S_{2} = \{\mathbf{v}_1 = \begin{bmatrix} 2 & 1 & 3 \end{bmatrix}, \mathbf{v}_2 = \begin{bmatrix} 5 & 1 & 5 \end{bmatrix}$, $\mathbf{v}_3 = \begin{bmatrix} 1 & 2 & 4 \end{bmatrix} 
...
Now the codeword of the given message $u = [\alpha, \alpha^{4}, \alpha^{5} ]$ is obtained for the given evaluation set $ \alpha*{1} = 1, \alpha*{2} = \alpha, \ldots , \alpha\_{6} = \alpha^{5} $ as follows:
